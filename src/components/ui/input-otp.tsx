...
const InputOTPSlot = React.forwardRef<React.ElementRef<'div'>, React.ComponentPropsWithoutRef<'div'> & { index: number }>(({ index, className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      // className={cn(
      //   'relative flex h-9 w-9 items-center justify-center border-y border-r border-input text-sm shadow-sm transition-all rounded-full',
      //   isActive && 'z-10 ring-1 ring-primary',
      //   className,
      // )}
      className={cn(
        'relative flex h-14 w-14 items-center justify-center border border-text-25 text-2xl transition-all rounded-full', // full rounded slot
        isActive && 'ring-2 ring-primary',
        char && 'border-primary text-primary',
        className,
      )}
      {...props}
    >
