...
import type { ClientDependent } from '../Types';
import Loader from '@/components/ui/loader/Loader';
import { Button } from '@/components/ui/button';

const AccessClientDependents = () => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {dependents.map((item) => (
            <div key={item.familyId} className="rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md transition">
              ...
              <Button
                onClick={() => navigate(`/user/access-account/${uuid}/dependents/${item?.familyId}`)}
                className="mt-4 w-full py-2 text-sm rounded-lg bg-primary text-white hover:bg-primary/90"
              >
                View Details
              </Button>
            </div>
...
export default AccessClientDependents;
